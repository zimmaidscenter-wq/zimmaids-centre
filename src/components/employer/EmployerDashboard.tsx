...
export const EmployerDashboard: React.FC = () => {
  ...
  // Choosing applicant & paying placement fee state
  const [isChoosingApplicant, setIsChoosingApplicant] = useState<string | null>(null);
  const [applicantChoiceFeedback, setApplicantChoiceFeedback] = useState<{ appId: string; type: "success" | "error"; text: string } | null>(null);

  const handleChooseAndUnlockApplicant = async (appId: string) => {
    setIsChoosingApplicant(appId);
    setApplicantChoiceFeedback(null);
    const res = await chooseAndUnlockApplicant(appId);
    setIsChoosingApplicant(null);
    if (res.success) {
      setApplicantChoiceFeedback({
        appId,
        type: "success",
        text: `Candidate successfully chosen! Placement fee of $${pricingSettings.premiumMaidAccessFeeUSD}.00 USD has been paid from your account. Direct phone number and WhatsApp contact are now unlocked below.`
      });
    } else {
      setApplicantChoiceFeedback({
        appId,
        type: "error",
        text: res.error || "Failed to choose applicant."
      });
    }
  };
                ...
                <div>
                  ...
                  <div className="relative h-48 bg-slate-100 overflow-hidden">
                    ...
                    <div className="absolute top-3 left-3 flex flex-wrap gap-1.5">
                      {maid.isNew && (
                        <span className="px-2.5 py-1 bg-teal-400 text-slate-950 text-[11px] font-black rounded-full flex items-center gap-1 shadow-sm">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white p-5 rounded-3xl border border-slate-200 shadow-xs">
            <div>
              ...
              <p className="text-xs text-slate-500 max-w-xl">
                All domestic workers apply directly to your dashboard. Choose your preferred candidate and pay the <strong>${pricingSettings.premiumMaidAccessFeeUSD}.00 USD</strong> placement fee from your account balance to unlock direct phone calling and WhatsApp contact.
              </p>
            </div>
            <div className="flex items-center gap-3 bg-slate-50 border border-slate-200/80 px-4 py-3 rounded-2xl shrink-0">
              <Wallet className="w-5 h-5 text-emerald-600" />
              <div>
                <div className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Account Balance</div>
                <div className="text-sm font-black text-slate-900">${currentWallet.balance.toFixed(2)} USD</div>
              </div>
              <button
                type="button"
                onClick={() => setIsAddFundsOpen(true)}
                className="ml-2 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-xl shadow-xs transition-all active:scale-95"
              >
                + Add Funds
              </button>
            </div>
          ...
          {employerApplications.length === 0 ? (
            <div className="bg-white rounded-3xl p-12 text-center border border-slate-200 space-y-3">
              ...
              <p className="text-xs text-slate-500 max-w-sm mx-auto">
                When workers apply for your posted vacancies, their profiles, vetting status, cover notes, and placement unlock options will appear here.
              </p>
            </div>
          ) : (
              ...
              {employerApplications.map((app) => (
                <div
                  ...
                  className={`bg-white rounded-3xl p-6 border shadow-xs transition-all ${
                    app.isUnlocked ? "border-emerald-200 ring-1 ring-emerald-500/20" : "border-slate-200"
                  }`}
                >
                  <div className="flex flex-col md:flex-row items-start justify-between gap-5">
                      ...
                      <div className="space-y-1.5">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <span
                            className={`px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider ${
                              app.isUnlocked || app.status === "Approved" || app.status === "Hired"
                                ? "bg-emerald-100 text-emerald-800"
                          ...
                          >
                            {app.isUnlocked ? "Chosen / Unlocked" : `Status: ${app.status}`}
                          </span>
                        </div>

                        <div className="text-xs text-slate-500 font-semibold">
                          Applied for: <strong className="text-slate-800">{app.jobTitle}</strong> ({app.jobLocation}) • Salary: <strong>${app.jobSalary} USD</strong>
                        </div>
                        ...
                        {app.applicantSkills && (
                          <div className="text-xs text-slate-600 flex items-center gap-1.5 flex-wrap">
                            ...
                            <span className="bg-slate-50 px-2 py-0.5 rounded-md border border-slate-200/80 text-[11px] font-medium text-slate-700">
                              {app.applicantSkills}
                            </span>
                          </div>
                        )}
                        ...
                        {app.coverNote && (
                          <p className="text-xs text-slate-600 italic bg-slate-50 p-3 rounded-2xl max-w-2xl border border-slate-100">
                            "{app.coverNote}"
                          </p>
                        )}
                        ...
                        <div className="flex items-center gap-3 text-[11px] font-medium text-slate-400 pt-1">
                          ...
                          {app.hasPoliceClearance && (
                            <span className="text-emerald-700 font-bold flex items-center gap-1">
                              <ShieldCheck className="w-3.5 h-3.5" /> Police Clearance on File
                            </span>
                          )}
                          {app.hasReferences && (
                    ...
                    {/* Quick Shortlist / Reject if not unlocked */}
                    {!app.isUnlocked && (
                      <div className="flex items-center gap-2 flex-wrap md:flex-col items-end shrink-0">
                        {app.status === "Pending" && (
                          <button
                            type="button"
                            onClick={() => updateApplicationStatus(app.id, "Shortlisted")}
                            className="px-3 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-200 font-bold text-xs rounded-xl transition-all"
                          >
                            Shortlist
                          </button>
                        )}
                        {app.status !== "Rejected" && (
                          <button
                            ...
                            className="px-3 py-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 font-bold text-xs rounded-xl transition-all"
                          >
                            Decline
                          </button>
                        )}
                  ...
                  {/* Feedback Message */}
                  {applicantChoiceFeedback && applicantChoiceFeedback.appId === app.id && (
                    <div
                      className={`mt-4 p-3.5 rounded-2xl text-xs flex items-center justify-between gap-3 ${
                        applicantChoiceFeedback.type === "success"
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold">{applicantChoiceFeedback.text}</span>
                      </div>
                      {applicantChoiceFeedback.type === "error" && (
                        <button
                          type="button"
                          onClick={() => setIsAddFundsOpen(true)}
                          className="px-3 py-1.5 bg-rose-600 hover:bg-rose-700 text-white font-black text-xs rounded-xl shadow-xs shrink-0 active:scale-95"
                        >
                          Top Up Balance Now
                        </button>
                      )}
                  ...
                  {/* PLACEMENT UNLOCK STATUS / ACTIONS */}
                    ...
                    {app.isUnlocked ? (
                      <div className="bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-2xl p-4 space-y-3">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              <h4 className="text-xs font-black text-emerald-950 uppercase tracking-wider">
                                Candidate Chosen • Placement Fee Paid
                              </h4>
                              <p className="text-[11px] text-emerald-700 font-medium">
                                Direct phone and WhatsApp details are fully unlocked. Contact candidate now to schedule start date.
                              </p>
                            </div>
                          </div>
                          <span className="px-2.5 py-1 bg-white text-emerald-800 text-[11px] font-black rounded-lg border border-emerald-200 shadow-2xs">
                            Placement Confirmed ✓
                          </span>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-3 pt-1">
                          <a
                            href={`tel:${app.applicantPhone || "+263771234567"}`}
                            className="px-4 py-2.5 bg-slate-900 hover:bg-slate-800 text-white font-bold text-xs rounded-xl flex items-center gap-2 transition-all shadow-sm active:scale-95"
                          >
                            <Phone className="w-3.5 h-3.5 text-emerald-400" />
                            <span>Call Candidate: {app.applicantPhone || "+263 77 123 4567"}</span>
                          </a>

                          <a
                            href={`https://wa.me/${(app.applicantWhatsApp || app.applicantPhone || "263771234567").replace(/[^0-9]/g, "")}?text=${encodeURIComponent(
                              `Hello ${app.maidName}, I am reaching out from Verified Maids Zimbabwe regarding your application for the ${app.jobTitle} position. We have chosen you for this placement and would like to confirm your start details.`
                            )}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-xl flex items-center gap-2 transition-all shadow-sm active:scale-95"
                          >
                            <MessageSquare className="w-3.5 h-3.5 text-white" />
                            <span>WhatsApp: {app.applicantWhatsApp || app.applicantPhone || "+263 77 123 4567"}</span>
                          </a>
                        </div>
                      </div>
                    ) : (
                      <div className="bg-amber-50/70 border border-amber-200/80 rounded-2xl p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div>
                            <div className="flex items-center gap-2">
                              <span className="text-xs font-black text-amber-950">Direct Contact Details Locked</span>
                              <span className="px-2 py-0.5 bg-amber-200/70 text-amber-900 text-[10px] font-black rounded-md">
                                ${pricingSettings.premiumMaidAccessFeeUSD}.00 USD Placement Fee
                              </span>
                            </div>
                            <p className="text-[11px] text-amber-800 mt-0.5 max-w-xl">
                              Choose this maid to hire them for your vacancy. The placement fee of <strong>${pricingSettings.premiumMaidAccessFeeUSD}.00 USD</strong> will be deducted from your account wallet balance, instantly revealing their phone and WhatsApp contacts.
                            </p>
                          </div>
                        </div>
                        ...
                        <button
                          type="button"
                          onClick={() => handleChooseAndUnlockApplicant(app.id)}
                          disabled={isChoosingApplicant === app.id}
                          className="w-full sm:w-auto px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-black text-xs rounded-xl shadow-md transition-all flex items-center justify-center gap-2 whitespace-nowrap active:scale-95 disabled:opacity-50"
                        >
                          <UserCheck className="w-4 h-4" />
                          <span>
                            {isChoosingApplicant === app.id
                              ? "Deducting Fee..."
                              : `Choose Maid & Pay $${pricingSettings.premiumMaidAccessFeeUSD} Fee`}
                          </span>
                        </button>
                      </div>
                    )}
